...
const Experience = () => {
  ...
  return (
    <section id="experience" className="bg-[var(--color-bg-light)] dark:bg-[var(--color-bg-dark)] py-24 md:py-32 lg:py-40 overflow-hidden border-t border-primary/10 transition-colors duration-700">
              ...
              <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 xl:gap-24 items-start">
                ...
                <div className="lg:col-span-4 space-y-4 md:space-y-8">
                  <div className="flex items-center space-x-4 text-primary">
                    <Calendar className="w-5 h-5 md:w-6 md:h-6 text-primary/40" />
                    <span className="text-2xl md:text-3xl font-display italic text-primary/40 dark:text-primary/20 group-hover:text-primary transition-colors font-medium">
                      {exp.period}
                    </span>
                  </div>
                  <div className="space-y-1.5 md:space-y-3">
                    <span className="text-[8px] md:text-[10px] font-mono tracking-[0.2em] md:tracking-[0.3em] text-primary/60 dark:text-primary/40 uppercase font-black">Organización</span>
                ...
                <div className="lg:col-span-8 space-y-8 md:space-y-12 mt-6 lg:mt-0">
                  <div className="space-y-4 md:space-y-6">
                    ...
                    <p className="text-lg md:text-xl lg:text-2xl font-display font-light text-black/70 dark:text-white/60 leading-relaxed max-w-3xl">
                      {exp.description}
                    </p>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-x-8 lg:gap-x-12 gap-y-6 md:gap-y-8 pt-4 md:pt-6">
                    {exp.achievements.map((item, i) => (
                      <div key={i} className="flex items-start space-x-3 md:space-x-4 group/item">
                        <ChevronRight className="w-4 h-4 md:w-5 md:h-5 mt-1 text-primary/30 group-hover/item:text-primary group-hover/item:translate-x-1 transition-all shrink-0" />
                        <p className="text-base md:text-lg lg:text-xl font-display font-light text-black/60 dark:text-white/50 leading-snug">
                          {item}
                        </p>
                      </div>
                    ))}
